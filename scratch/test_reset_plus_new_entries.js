...
function computeAccountBalances(transactions, baselineAccounts = []) {
  ...
  // 1. Identify index where ResetData starts
  let resetStartIndex = -1;
  ...
  // If ResetData exists, take transactions starting from resetStartIndex
  const activeTransactions = resetStartIndex !== -1 ? transactions.slice(resetStartIndex) : transactions;
...
async function test() {
  ...
  console.log("\n=== ACTIVE ACCOUNT BALANCES (FROM RESETDATA FORWARD) ===");
  accs.forEach(a => {
    console.log(`${a.name.padEnd(16)} [${a.type.padEnd(12)}] [${a.currency}] : Balance: ${a.liveBalance >= 0 ? '+' : ''}${a.liveBalance.toLocaleString()} | Inflow: +${a.incomeSum + a.transferIn} | Outflow: -${a.expenseSum + a.transferOut}`);
  });
